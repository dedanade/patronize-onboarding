  ...
  const handleBVNOnchange = (value) => {
    if (value.length === 11) {
      if (errors?.bvnInput) {
        ...
        newErrors.bvnInput = null;
        setErrors(newErrors);
      }
      if (!onboardForm) {
        setOnboardForm({ bvnInput: value });
      } else {
        const newOnboardForm = { ...onboardForm };
        newOnboardForm.bvnInput = value;
        setOnboardForm(newOnboardForm);
      }
    } else {
      if (onboardForm?.bvnInput) {
        const newOnboardForm = { ...onboardForm };
        newOnboardForm.bvnInput = null;
        setOnboardForm(newOnboardForm);
      }
      if (!errors) {
          ...
          {showBVNInput ? (
            <form onSubmit={(e) => handleSubmit(e)}>
              <div style={{ marginTop: '25px' }}>
                ...
                />
                {errors?.bvnInput && (
                  <span className='input-error'>{errors?.bvnInput}</span>
                )}
              </div>
